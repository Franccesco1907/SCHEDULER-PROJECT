  ...
  $(document).on("click", "#btn-add-new-request", async function () {
    ...
    if (motivo != "" && plazas > 0) {
      ...
      let response = await postData(
        `plazas/plazadocentecontratado/guardar`, new_request, jwt, user
      );
    ...
    if (editable == 0) {
      ...
      if (motivo != "" && plazas > 0) {
        ...
        let response = await putData(
          `plazas/plazadocentecontratado/actualizarprevalor`, new_request, jwt, user
        );
        ...
        if (response.data) {
          ...
          swal("¡La solicitud ha sido editada correctamente!", "Espera a los resultados del proceso", "success");
          location.reload();
  ...
  if (procesoActivo) {
    ...
    if (today < beginning) {
      ...
      if (flag == true) {
        ...
        //const b = document.getElementById("btn-add-new-request");
        //b.innerText = "Editar";justRequest
