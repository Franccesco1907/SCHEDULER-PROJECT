...
$(async function () {
    ...
    $('#to-date').hide();
    $('.select2').select2();
    await loadRequestsFilters(user.idDepartamento);
    ...
    $('#apply-filters').on('click', async function () {
        ...
        await loadRequestsFilters(user.idDepartamento, status, asunto, remitente, fechaFin, fechaInicio);
    });
    ...
    $('#view-request').on('click', function () {
        if (idRequest) {
            ...
            let requestSelect = requests.find(x => x.idSolicitudesPersona === +id);
...
async function loadRequestsFilters(idDepartamento = 0, status = '', asunto = '', remitente = '', fechaFin = '', fechaInicio = '') {
    let requestData = [];

    if (fechaFin === '' && fechaInicio === '') requestData = await getData(`mesa/solicitudespersona/listarpordepartamento?idDepartamento=${idDepartamento}&tipoSolicitudPersona=RECIBIDO&tamanioPag=2000&estado=${status}&asuntoOsolicitud=${asunto}&remitente=${remitente}`, jwt);
    else requestData = await getData(`mesa/solicitudespersona/listarpordepartamento?idDepartamento=${idDepartamento}&tipoSolicitudPersona=RECIBIDO&fechaInicio=${fechaInicio}&fechaFin=${fechaFin}&tamanioPag=2000&estado=${status}&asuntoOsolicitud=${asunto}&remitente=${remitente}`, jwt);
    ...
    let options = {
        ...
        data: requests,
        fields: ['nombreEmisor', 'derivadoA', 'asunto', 'tipoSolicitud', 'fecha', 'estadoSolicitud'],
        name: 'Trámites',
        idName: 'idSolicitudesPersona',
