...
$(async function () {
  ...
  let selectedRequestPlacesAux = undefined;
  ...
  $(document).on("click", ".requests-places-new-teachers-row", function () {
    if (
      ...
      selectedRequestPlacesAux = parseInt($(this).children().eq(5).html());
  ...
  $("#edit-request-places-new-teachers").on("click", async function () {
    ...
    let placesCouncilAux = $("#requested-approved-places-council-aux").val();
    let placesDapAux = $("#requested-approved-places-dap-aux").val();
    ...
    let response2 = await putData(
      `plazas/plazaordinaria/actualizarpostvalor`, {
      "plazain": { "idPlaza": id, "cantidadAceptadas": placesCouncil, "numeroAprobadasDAP": placesDap },
      "esOrdinario": est, "cantidadAceptadasAuxiliar": placesCouncilAux, "numeroAprobadasDAPAuxiliar": placesDapAux
    }, $jwt.val(), user
    );
    if (response2) {
  ...
  $("#edit-request-places-new-teachers-modal").on("click", function () {
    ...
    if (idRequestPlacesNewTeachers) {
      ...
      $("#requested-hours-aux").val(`${selectedRequestPlacesAux}`);
      let id = parseInt(idRequestPlacesNewTeachers.split("-")[1]);
  ...
  let today = new Date();
  let procesos = await getData(`plazas/procesosplazas/listar?tamanioPag=2000&tipoProcesoPlaza=ASCENSO&idDepartamento=${idDepartamento}`, $jwt.val());
  ...
  $(".initp").on("click", async function () {
    ...
    let response3 = await postData(
      `plazas/procesosplazas/guardar`, { "fechaInicio": aboutToDateBe, "fechaFin": aboutToDateEnd, "estado": true, "tipoProcesoPlaza": "ASCENSO", "idDepartamento": idDepartamento }, $jwt.val(), user
    );
    location.reload();
  });
...
async function loadPlacesNewTeachers() {
  placesNewTeachersData = await getData(`plazas/plazaordinaria/listarascenso?tamanioPag=2000&idProcesoPlaza=${idProceso}`, $jwt.val()
  );
  //console.log("DATAAAA RAAAAAAAAA");
  ...
  let options = {
    ...
    fields: [
      ...
      "aprobadasPorDAP",
      "cantidadSolicitadasAuxiliar",
      "aprobadasPorDepartamentoAuxiliar",
      "aprobadasPorDAPAuxiliar"
    ],
