...
$(async function () {
  ...
  $("#edit-request-places-new-teachers").on("click", async function () {
    ...
    let response2 = await putData(
      `plazas/plazadocentecontratado/actualizarpostvalor`, { "plazain": { "idPlaza": id, "cantidadAceptadas": placesCouncil, "numeroAprobadasDAP": placesDap } }, $jwt.val(), user
    );
    if (response2) {
  ...
  $("#edit-request-places-new-teachers-modal").on("click", function () {
    ...
    if (idRequestPlacesNewTeachers) {
      ...
      let id = parseInt(idRequestPlacesNewTeachers.split("-")[1]);
      let wantedP = placesNewTeachersData.data.find(x => x.idPlazaDocente == id);
  ...
  let today = new Date();
  let procesos = await getData(`plazas/procesosplazas/listar?tamanioPag=2000&tipoProcesoPlaza=NUEVO_DOCENTE&idDepartamento=${idDepartamento}`, $jwt.val());
  ...
  $(".initp").on("click", async function () {
    ...
    let response3 = await postData(
      `plazas/procesosplazas/guardar`, { "fechaInicio": aboutToDateBe, "fechaFin": aboutToDateEnd, "estado": true, "tipoProcesoPlaza": "NUEVO_DOCENTE", "idDepartamento": idDepartamento }, $jwt.val(), user
    );
    location.reload();
  });
...
async function loadPlacesNewTeachers() {
  placesNewTeachersData = await getData(`plazas/plazadocentecontratado/listar?tamanioPag=2000&idProcesoPlaza=${idProceso}`, $jwt.val()
  );
  let options = {
...
function formattingStringDate(stringDate) {
  ...
  //console.log("Luego de formatear:");
  //console.log(stringDate);
