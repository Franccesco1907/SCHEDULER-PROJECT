...
async function loadRequests(idPersona) {
    let requestData = await getData(`mesa/solicitudespersona/listarrecibidos?idPersona=${idPersona}&tamanioPag=2000`, jwt);
...
async function loadRequestsFilters(idPersona, status, asunto, remitente, fechaFin, fechaInicio) {
    ...
    if (fechaFin === '' && fechaInicio === '') requestData = await getData(`mesa/solicitudespersona/listarrecibidosfiltros?idPersona=${idPersona}&tamanioPag=2000&estado=${status}&asuntoOsolicitud=${asunto}&remitente=${remitente}`, jwt);
    else requestData = await getData(`mesa/solicitudespersona/listarrecibidosfiltros?idPersona=${idPersona}&fechaInicio=${fechaInicio}&fechaFin=${fechaFin}&tamanioPag=2000&estado=${status}&asuntoOsolicitud=${asunto}&remitente=${remitente}`, jwt);
